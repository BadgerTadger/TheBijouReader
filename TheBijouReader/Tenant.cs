...
public class Tenant
{
    ...
    {
        return GetTenantData(0);
    }

    public DataSet GetTenantData(int sortColumn)
    {
        DataSet retVal = null;
        try
        {
            string sqlCmd = @"SELECT TenantID, SUBSTRING_INDEX(SUBSTRING_INDEX(TenantName, ' ', 1), ' ', -1) AS first_name,
                           If(  length(TenantName) - length(replace(TenantName, ' ', ''))>1,  
                               SUBSTRING_INDEX(SUBSTRING_INDEX(TenantName, ' ', 2), ' ', -1) ,NULL) 
                                   as middle_name,
                           SUBSTRING_INDEX(SUBSTRING_INDEX(TenantName, ' ', 3), ' ', -1) AS last_name, 
                            Address1, Address2, Town, City, County,
                            PostCode, Country, Email, Landline, Mobile, Comments, PreviousTenant FROM thebijouvilla.Tenants";

            if(sortColumn > 0)
            {
                sqlCmd += string.Format(" ORDER BY {0} ", sortColumn);
            }
